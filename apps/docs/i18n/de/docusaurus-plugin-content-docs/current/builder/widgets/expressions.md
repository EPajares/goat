...
Diese Seite hilft Ihnen zu verstehen, wie Sie Ausdrücke im [Dashboard Builder](/builder/widgets.md) verwenden. Sie können Ausdrücke in den **Zahlen-, Kategorien-** und **Kreisdiagramm-Widgets** eingeben. Wir listen die Ausdrücke auf und beschreiben sie, die Sie in GOAT verwenden können.
...
Wenn Sie sich auf ein Feld beziehen, fügen Sie es nur in Klammern ein: **(Feldname)**.

:::

## Numerische Funktionen
Die folgenden Funktionen funktionieren nur mit Zahlenfeldern. Die Eingabe und die Ausgabe sind numerische Werte.
...
| **pow(x, y)**  | erhebt *x* zur Potenz von *y*  | <button>pow(2, 3)</button> = 8 |
| **exp(x)**  | gibt *e* hoch *x* zurück| <button>exp(1)</button> = 2,718... |
...
| **sin(x)**  | gibt den Sinus von *x* (Radiant) zurück | <button>sin(1)</button> = 0.841... |
| **cos(x)**  | gibt den Kosinus von *x* (Radiant) zurück | <button>cos(1)</button> = 0.541... |
| **tan(x)**  |  gibt den Tangens von *x* (Radiant) zurück | <button>tan(0.75)</button> = 0.932... |
| **asin(x)**  |  gibt den Arkussinus von *x* (Radiant) zurück | <button>asin(1)</button> = 1.571... |
| **acos(x)**  |  gibt den Arkuskosinus von *x* (Radiant) zurück | <button>acos(0.5)</button> = 1.047... |
| **atan(x)**  |  gibt den Arkustangens von *x* (Radiant) zurück | <button>atan(1)</button> = 0.785... |
| **degrees(x)**  | wandelt Winkel x *von Radiant in Grad* um | <button>degrees(1)</button> = 57.296... |
| **radians(x)**  | wandelt Winkel x *von Grad in Radiant* um | <button>radians(180)</button> = 3.142... |
| **rand(x, y)**  | erzeugt eine Zufallszahl zwischen x und y | <button>rand(1, 5)</button> = 3.17 |

## String-Funktionen
Die folgenden Funktionen funktionieren nur mit Textfeldern. Die Eingabe und die Ausgabe sind Textwerte.
...
| **upper(string)**  | wandelt alle Buchstaben in *Großbuchstaben* um  | <button>upper(walking)</button> = WALKING |
| **lower(string)**  | wandelt alle Buchstaben in *Kleinbuchstaben* um | <button>lower(GOAT)</button> = goat |
...
| **substr(string, start, length)**  | gibt einen Teilstring zurück, beginnend an *start*-Position mit optionaler *Länge* | <button>substr(mobility, 1, 3)</button> = mob |
| **substring(string, start, length)**  |  gibt einen Teilstring zurück, beginnend an *start*-Position mit optionaler *Länge* | <button>substring(mobility, 1, 3)</button> = mob |
| **left(string, n)**  | gibt die *n Zeichen ganz links* zurück | <button>left(accessibility, 4)</button> = acce |
| **right(string, n)**  | gibt die *n Zeichen ganz rechts* zurück | <button>right(accessibility, 4)</button> = lity |
...
| **regexp_substr(string, pattern)**  | extrahiert den *ersten Teilstring, der einem regulären Ausdruck entspricht* |  <button>regexp_substr(StopID: 45B, 0-9+)</button> = 45  |
| **strpos(string, substring)**  | gibt die *Position (Index) zurück, an der ein Teilstring zuerst erscheint* und gibt 0 zurück, wenn nicht gefunden |  <button>strops(WalkScore, Score)</button> = 5  |
| **concat(a,b...)**  | *verbindet mehrere Strings oder Felder* miteinander |    |

## Datum-Zeit-Funktionen

Die folgenden Funktionen funktionieren nur mit Datum/Zeit-Feldern. Die Eingabe und die Ausgabe sind Datum/Zeit-Werte.

| Ausdruck | Funktion  |  Beispiel |
|------------|-----------|----------|
| **now()**  | gibt das *aktuelle Datum und die aktuelle Zeit* zurück |  <button>now()</button> = 2025-10-07 11:35:00 |
...
| **make_time(hour, minute, second)**  | erstellt eine Zeit aus numerischer Stunde, Minute, Sekunde | <button>make_time(14, 30, 0)</button> =  14:30:00 |
| **make_timestamp(year, month, day, hour, minute, second)**  | kombiniert Datum und Zeit zu einem Zeitstempel  | <button>make_timestamp(2025, 10, 8, 14, 30, 0)</button> = 2025-10-08 14:30:00 |
| **to_date(string, format)**  | wandelt einen Textstring in ein Datum um unter Verwendung des angegebenen Formats  | <button>to_date('08/10/2025','DD/MM/YYYY')</button> = 2025-10-08  |
| **to_timestamp(string, format)**  | wandelt einen Textstring in einen Zeitstempel um unter Verwendung des angegebenen Formats  |  <button>to_timestamp('08/10/2025 14:30','DD/MM/YYYY HH24:MI')</button> = 2025-10-08 14:30:00 |
| **to_char(date, format)**  | wandelt ein Datum oder einen Zeitstempel in einen formatierten Textstring um  | <button>to_char("survey_date",'YYYY-MM-DD')</button> = '2025-10-08' |

## Konvertierungsfunktionen

Die folgenden Funktionen wandeln einen Wert von einem Typ in einen anderen um. Die Eingabe und die Ausgabe sind von verschiedenen Typen.

| Ausdruck | Funktion  |  Beispiel |
|------------|-----------|----------|
| **to_int(x)**  | wandelt x in eine Ganzzahl um | <button>to_int(3.9)</button> = 3 |
| **to_real(x)**  | wandelt *x* in eine **reelle (Dezimal-)Zahl** um | <button>to_real('3.9')</button> = 3.9 |
| **to_string(x)**  |  wandelt *x* in einen **Textstring** um | <button>to_string(25)</button> = '25' |

## Allgemeine Funktionen

Die folgenden Funktionen funktionieren mit jedem Feldtyp. Die Eingabe und die Ausgabe sind vom gleichen Typ.
...
| **avg(field)** | gibt den **Durchschnittswert (Mittelwert)** eines Feldes zurück | <button>avg("travel_time")</button> = 12.6 |
| **min(field)** | gibt den **kleinsten (minimalen)** Wert in einem Feld zurück | <button>min("distance")</button> = 0.5 |
| **max(field)** | gibt den **größten (maximalen)** Wert in einem Feld zurück | <button>max("distance")</button> = 18.2 |
...
| **ST_Length(geometry)** | gibt die **Länge einer gegebenen Geometrie** zurück (Linie oder Polygon-Grenze) | <button>ST_Length($geometry)</button> = 275.3 |
| **perimeter** | gibt die **Umfangslänge** eines Polygon-Features zurück | <button>perimeter($geometry)</button> = 490.6 |
| **ST_Perimeter(geometry)** | gibt den **Umfang** einer Geometrie zurück, ähnlich wie `perimeter()`, aber nach dem **PostGIS-Standard-Namensschema** | <button>ST_Perimeter($geometry)</button> = 490.6 |

## Metrische Puffer-Funktion

| Ausdruck | Funktion | Beispiel |
|------------|-----------|----------|
| **buffer(geometry, distance)** | erstellt einen **Polygon-Puffer** um eine Geometrie in der angegebenen *Entfernung* (in Layer-Einheiten, z.B. Meter) | <button>buffer($geometry, 100)</button> = Polygon, das eine 100 m Pufferzone darstellt |

## Geometrische unäre Funktionen
...
| **ST_Centroid(geometry)** | dasselbe wie `centroid()`, folgt der **PostGIS-Namenskonvention** | <button>ST_Centroid($geometry)</button> = Point(13.41, 52.52) |
...
| **ST_ConvexHull(geometry)** | dasselbe wie `convex_hull()`, unter Verwendung der **PostGIS-Syntax** | <button>ST_ConvexHull($geometry)</button> = Polygon(...) |
| **envelope(geometry)** | gibt das **minimale umhüllende Rechteck** einer Geometrie zurück | <button>envelope($geometry)</button> = Polygon((xmin, ymin), (xmax, ymax)) |
...
| **is_empty(geometry)** | gibt **TRUE zurück, wenn die Geometrie keinen räumlichen Inhalt hat** | <button>is_empty($geometry)</button> = FALSE |
| **ST_IsEmpty(geometry)** | dasselbe wie `is_empty()`, **PostGIS-Form** | <button>ST_IsEmpty($geometry)</button> = FALSE |
| **is_valid(geometry)** | gibt **TRUE zurück, wenn die Geometrie gültig ist** | <button>is_valid($geometry)</button> = TRUE |
